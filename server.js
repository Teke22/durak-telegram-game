...
function startGame(session) {
  ...
  session.roundMax = null; // <-- максимальное число атак в текущем раунде (фиксируется при первой атаке)
...
app.post('/api/join-game/:gameId', (req, res) => {
  try {
    ...
    // Если id уже есть — добавим с суффиксом чтобы позволить дубликаты (как раньше)
...
app.get('/api/game/:gameId', (req, res) => {
  try {
    ...
    const base = {
      ...
      roundMax: session.roundMax // <-- возвращаем client'у
    };
...
app.post('/api/game/:gameId/move', (req, res) => {
  try {
    ...
    if (action === 'attack') {
      ...
      // New logic: если это первая атака в раунде — фиксируем session.roundMax = min(6, defenderHandCount)
      if (session.table.length === 0) {
        const defenderHandCount = session.hands[defender].length;
        session.roundMax = Math.min(6, defenderHandCount);
      }

      // limit: number of cards on table must be < session.roundMax (если задан), иначе fallback на defender hand size
    ...
    if (action === 'add') {
      ...
      // ensure session.roundMax exists (should be set on first attack), fallback to defender hand size
      const limit = (typeof session.roundMax === 'number' && session.roundMax > 0) ? session.roundMax : session.hands[defender].length;
    ...
    if (action === 'take') {
      ...
      session.table = [];
      // Сброс roundMax после взятия — новый раунд начнётся со свежим лимитом при следующей атаке
    ...
    if (action === 'pass') {
      ...
      session.table = [];
      // Сброс roundMax после успешного бита
      session.roundMax = null;
