...
app.post('/api/game/:gameId/move', (req, res) => {
  try {
    ...
    if (action === 'attack'){
      ...
      // set roundMax on first attack in round
...
function registerHandlers(b){
  ...
  b.onText(/\/start/, (msg) => {
    try {
      ...
      const appUrl = APP_URL || `https://t.me`; // filler if not available
      const keyboard = {
        inline_keyboard: [
...
async function initBot(){
  ...
  // webhook attempt if APP_URL present
  if (APP_URL){
    try {
      ...
      // add express endpoint for webhook
