// public/script.js
// Полный клиентский скрипт: надёжная логика локального бота (атака/защита), сортировка рук,
// подсветка козырей, inline join UI и мультиплеерные вызовы (без изменений).
// Замените текущий public/script.js этим файлом.
...
const SUIT_ORDER = ['♣','♦','♥','♠']; // сортировка по мастям (для не-козырей)
...
// Сортировка руки: не-козыри по масти и рангу (возрастание), козыри в конце (по рангу)
function sortHand(hand, trumpSuit){
  hand.sort((a,b) => {
    ...
    return rankIndex(a.rank) - rankIndex(b.rank); // both trumps
  });
}

/* ---------------- Local bot implementation (robust) ---------------- */
const LOCAL = {
  deck: [], playerHand: [], botHand: [],
  table: [], trumpSuit: null, trumpCard: null,
  attacker: 'player', defender: 'bot', currentPlayer: 'player',
  phase: 'attacking', status: 'idle', roundMax: null
};
...
function initLocalGame(){
  ...
  for (let i=0;i<6;i++){
    if (LOCAL.deck.length) LOCAL.playerHand.push(LOCAL.deck.pop());
    if (LOCAL.deck.length) LOCAL.botHand.push(LOCAL.deck.pop());
  }
  LOCAL.table = []; LOCAL.attacker = 'player'; LOCAL.defender = 'bot';
  LOCAL.currentPlayer = LOCAL.attacker; LOCAL.phase = 'attacking'; LOCAL.status = 'playing';
  LOCAL.roundMax = null;
...
function renderTableLocal(){
  ...
  LOCAL.table.forEach(pair => {
    ...
    wrap.appendChild(cardNode(pair.attack, false, LOCAL.trumpSuit));
    if (pair.defend){ const d = cardNode(pair.defend, false, LOCAL.trumpSuit); d.classList.add('defended'); wrap.appendChild(d); }
...
function renderActionButtonsLocal(){
  ...
  if (LOCAL.phase === 'defending' && LOCAL.currentPlayer === 'player'){
    const take = document.createElement('button'); take.className = 'secondary danger'; take.textContent = 'Взять'; take.onclick = takeCardsLocal;
    actions.appendChild(take);
  }

  // Показываем "Бито" только когда все пары защищены и игрок — атакующий, и ему нужно подтвердить
  if (allDef && LOCAL.currentPlayer === 'player' && LOCAL.attacker === 'player'){
    const b = document.createElement('button'); b.className = 'secondary'; b.textContent = 'Бито'; b.onclick = () => { passLocal(); };
...
function renderPlayerHandLocal(){
  ...
  sec.innerHTML = '<h3>Ваши карты:</h3>';
  ...
  LOCAL.playerHand.forEach((card, idx) => {
    ...
    const n = cardNode(card, clickable, LOCAL.trumpSuit);
...
function cardNode(card, clickable, trumpSuit){
  const d = document.createElement('div');
  d.className = 'card' + (clickable ? ' clickable' : '') + (card.suit === trumpSuit ? ' trump' : '');
...
/* Game rule checks for local mode */
...
function attackLocal(index){
  ...
  renderAllLocal();
  // Гарантированный вызов бота после небольшой задержки (позволяет UI отрисоваться)
  setTimeout(() => { try { botMoveLocal(); } catch(e) { console.error('botMoveLocal error', e); } }, 180);
}
...
function defendLocal(index){
  ...
  if (allDef){
    // defender protected all pairs -> attacker may add more or press 'Бито'
    LOCAL.phase = 'attacking';
    LOCAL.currentPlayer = LOCAL.attacker;
    sortHand(LOCAL.playerHand, LOCAL.trumpSuit);
    renderAllLocal();
    // НЕ делаем refill: ждем решения атакующего (подкинуть/Бито)
  } else {
    // ещё есть пары без защиты -> ход переходит боту защищаться дальше
    LOCAL.currentPlayer = 'bot';
    sortHand(LOCAL.playerHand, LOCAL.trumpSuit);
    renderAllLocal();
    setTimeout(() => { try { botMoveLocal(); } catch(e) { console.error('botMoveLocal error', e); } }, 160);
  }
}

function takeCardsLocal(){
  for (const p of LOCAL.table){
    LOCAL.playerHand.push(p.attack);
    if (p.defend) LOCAL.playerHand.push(p.defend);
  }
  LOCAL.table = [];
  LOCAL.roundMax = null;
  refillLocal(true);
...
function refillLocal(defenderTook){
  ...
  sortHand(LOCAL.playerHand, LOCAL.trumpSuit); sortHand(LOCAL.botHand, LOCAL.trumpSuit);
  renderAllLocal();
  if (LOCAL.currentPlayer === 'bot') setTimeout(() => { try { botMoveLocal(); } catch(e) { console.error('botMoveLocal error', e); } }, 240);
}

/* Bot logic (reliable) */
function botMoveLocal(){
  // Отладка (можно отключить)
  // console.log('[BOT] phase=', LOCAL.phase, 'currentPlayer=', LOCAL.currentPlayer, 'attacker=', LOCAL.attacker);

  if (LOCAL.phase === 'attacking'){
    // Бот как атакующий
    if (LOCAL.table.length === 0){
      if (LOCAL.botHand.length === 0){ passLocal(); return; }
      // выбрать минимальную по "стоимости" карту: ненужные/слабые первые (не-козыри предпочтительнее)
      ...
      // бот может подкинуть карту (если есть та же достоинство и место)
      const ranks = new Set(); LOCAL.table.forEach(p=>{ ranks.add(p.attack.rank); if (p.defend) ranks.add(p.defend.rank); });
      const defenderCap = LOCAL.defender === 'player' ? LOCAL.playerHand.length : LOCAL.botHand.length;
      if (LOCAL.table.length < defenderCap){
        for (let i=0;i<LOCAL.botHand.length;i++){
      ...
      // не может подкинуть -> завершает ход
      passLocal();
      return;
    }
  } else if (LOCAL.phase === 'defending'){
    // Бот как защитник -> защищаемся от последней атакующей карты
    const last = LOCAL.table[LOCAL.table.length - 1];
    if (!last || last.defend){
      // нет чего защищать
      LOCAL.phase = 'attacking';
      LOCAL.currentPlayer = LOCAL.attacker;
      renderAllLocal();
      if (LOCAL.currentPlayer === 'bot') setTimeout(() => { try { botMoveLocal(); } catch(e) { console.error(e); } }, 200);
      return;
    }

    // Сначала ищем минимальную карту той же масти, старшую
    let sameIdx = -1, sameVal = Infinity;
    // Потом минимальную козырную
    let trumpIdx = -1, trumpVal = Infinity;
    ...
    let defendIdx = -1;
    if (sameIdx !== -1) defendIdx = sameIdx;
    else if (trumpIdx !== -1) defendIdx = trumpIdx;

    if (defendIdx === -1){
      // не можем защититься -> взять все
      for (const p of LOCAL.table){
      ...
      // console.log('[BOT] defended last with', defendCard);
      const allDef = LOCAL.table.every(p => p.defend);
      if (allDef){
        // Правильно: бот отбил — не меняем роли, даём атакующему шанс подкинуть или нажать бито.
        ...
        // Нужно защитить дальше — назначаем ход игроку
    ...
    // неизвестная фаза
    // console.warn('[BOT] unknown phase', LOCAL.phase);
  }
}
...
/* ---------------- Multiplayer (unchanged from last working) ---------------- */
