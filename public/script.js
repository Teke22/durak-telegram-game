...
let gameState = {
  ...
  // локальная одиночка (1×1 против бота)
  deck: [],
  playerHand: [],
  botHand: [],
  ...
  // для визуализации «текущего оппонента»
  opponentId: null,
  opponentCount: 0,
};
...
async function createMultiplayerGame({ maxPlayers, botCount, autostart, startWhenFull }) {
  try {
    ...
    const cfgText = `Комната создана • Мест: ${maxPlayers}, ботов: ${botCount}, ${startWhenFull ? 'ждём всех' : 'старт при готовности'}`;
    gameBoard.innerHTML = `
      <div class="panel" style="text-align:center;">
...
function applyServerState(s) {
  ...
  // визуализируем «текущего оппонента»
  let opponentId = null;
...
function renderSeats() {
  ...
  // сдвигаем список так, чтобы «вы» были первыми
...
function renderOpponentHand() {
  ...
  if (!gameState.isMultiplayer) {
    ...
    const title = (gameState.rawCurrent === gameState.rawDefender) ? 'Защитник' : 'Текущий соперник';
    const section = document.createElement('div');
    section.className = 'opponent-section';
  ...
  gameState.playerHand.forEach((card, index) => {
    ...
    if (clickable) {
      el.addEventListener('click', (ev) => {
        ...
        if (gameState.isMultiplayer) {
          if (canDefend && canDefendWithCardLocal(card)) {
            sendMove('defend', { rank: card.rank, suit: card.suit });
          } else if (canAttack && canAttackWithCardLocal(card)) {
            sendMove('attack', { rank: card.rank, suit: card.suit });
          } else if (canAdd) {
            sendMove('add', { rank: card.rank, suit: card.suit });
          }
        } else {
